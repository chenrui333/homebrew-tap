...
The Formula Metadata Crawler is an automated system that extracts formula metadata and fetches git hosting statistics for all formulas in the homebrew-tap. It generates a comprehensive `formula-status/formula-status.md` report sorted by popularity (stars).
...
Cache results (.cache/formula_metadata.json)
    ↓
Generate sorted Markdown table (formula-status/formula-status.md)
```
...
# With options
python3 formula-status/generate_formula_status.py \
  --output formula-status/formula-status.md \
  --workers 20 \
  --verbose \
  --refresh
```
...
- **Steps**:
  ...
  4. Commit changes if formula-status/formula-status.md updated
...
docs/
  └── FORMULA_formula-status/formula-status.md
```
...
- Check formula-status/formula-status.md for formula-specific data
