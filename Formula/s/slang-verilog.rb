class SlangVerilog < Formula
  desc "SystemVerilog compiler and language services"
  homepage "https://github.com/MikePopoloski/slang"
  url "https://github.com/MikePopoloski/slang/archive/refs/tags/v8.0.tar.gz"
  sha256 "eb776f050966489c68db99b2bee63cfa2227f812ed2e2bb1ed0247d267ab53c6"
  license "MIT"
  ...
  def install
    system "cmake", "-S", ".", "-B", "build", "-DSLANG_INCLUDE_TESTS=OFF", *std_cmake_args
    system "cmake", "--build", "build"
    system "cmake", "--install", "build"
  end

  test do
    assert_match version.to_s, shell_output("#{bin}/slang --version")
    ...
    output = shell_output("#{bin}/slang test.sv")
...
__END__
diff --git a/Formula/s/slang-verilog.rb b/Formula/s/slang-verilog.rb
index ac66000..3e7012b 100644
--- a/Formula/s/slang-verilog.rb
+++ b/Formula/s/slang-verilog.rb
@@ -41,282 +41,3 @@ class SlangVerilog < Formula
 end
 
 __END__
-diff --git a/external/CMakeLists.txt b/external/CMakeLists.txt
-index 93db349..de3b3d4 100644
---- a/external/CMakeLists.txt
-+++ b/external/CMakeLists.txt
-@@ -9,39 +9,43 @@ set(mimalloc_min_version "2.1")
...
-+if(NOT fmt_FOUND)
-+  message(STATUS "fmt package not found via find_package; falling back to FetchContent.")
-+  set(FMT_SYSTEM_HEADERS ON CACHE INTERNAL "")
-
--set(FMT_SYSTEM_HEADERS
--    ON
--    CACHE INTERNAL "")
-+  # For CMake 3.24+ we can pass version arguments to FetchContent_Declare
-+  set(find_pkg_args "")
-+  if(${CMAKE_VERSION} VERSION_GREATER_EQUAL "3.24.0")
-+    set(find_pkg_args "FIND_PACKAGE_ARGS" "${fmt_min_version}")
-+  endif()
...
-+  # Force fmtlib to always build as a static library when fetched,
-+  # since it's a private dependency that's mostly header-only.
-+  set(saved_build_shared_libs ${BUILD_SHARED_LIBS})
-+  set(BUILD_SHARED_LIBS OFF)
-+  FetchContent_MakeAvailable(fmt)
-+  set(BUILD_SHARED_LIBS ${saved_build_shared_libs})
...
-+  message(FATAL_ERROR "Could not find or fetch fmt package, min version: ${fmt_min_version}")
- endif()
-
-+# Ensure POSITION_INDEPENDENT_CODE is set if needed.
- if((SLANG_INCLUDE_PYLIB OR BUILD_SHARED_LIBS) AND NOT fmt_FOUND)
-   set_target_properties(fmt PROPERTIES POSITION_INDEPENDENT_CODE ON)
- endif()
-
-+# Inform the user whether a system or remote fmt is being used.
- if(fmt_FOUND)
...
-index 84e0320..bfdcfc1 100644
---- a/source/CMakeLists.txt
-+++ b/source/CMakeLists.txt
-@@ -129,11 +129,23 @@ target_include_directories(
-
- # Link libraries
...
-+    target_link_libraries(slang_slang PUBLIC mimalloc::mimalloc)
-+  elseif(TARGET mimalloc)
-+    target_link_libraries(slang_slang PUBLIC mimalloc)
-+  elseif(TARGET mimalloc::mimalloc-static)
-+    target_link_libraries(slang_slang PUBLIC mimalloc::mimalloc-static)
-+  elseif(TARGET mimalloc-static)
-+    target_link_libraries(slang_slang PUBLIC mimalloc-static)
-+  else()
-+    message(FATAL_ERROR "Could not find an appropriate mimalloc target")
-+  endif()
