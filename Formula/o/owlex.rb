class Owlex < Formula
  ...
  desc "MCP server for Codex CLI and Gemini CLI integration with Claude Code"
  homepage "https://github.com/agentic-mcp-tools/owlex"
  url "https://github.com/agentic-mcp-tools/owlex/archive/refs/tags/v0.1.10.tar.gz"
  sha256 "6e67d07aa3b3dafee9d7a5b271c3c94e7e507d4deb241891b755cef000ed3c20"
  license "MIT"
  ...
  test do
    ...
    version_output = shell_output(
      "#{python} -c \"import importlib.metadata as m; print(m.version('owlex'))\"",
    )
    assert_match version.to_s, version_output

    status_output = shell_output(
      "#{python} -c \"import json; from owlex.server import get_council_status; " \
      "status = json.loads(get_council_status()); print(status['running_count']); print(status['total_count'])\"",
    )
    assert_equal "0\n0\n", status_output
  end
end
