class WutCli < Formula
  ...
  def install
    ...
    site_packages.install buildpath/"wut"

    (bin/"wut").write <<~SH
      #!/bin/bash
      export PYTHONPATH="#{site_packages}:$PYTHONPATH"
      exec "#{libexec}/bin/python3.13" -m wut.wut "$@"
    SH
  end
  ...
  test do
    (testpath/"test.py").write <<~PY
      ...
      wut_mod.Console = lambda: fake_console
      sys.argv = ["wut"]
      os.environ.pop("TMUX", None)
      os.environ.pop("STY", None)
      wut_mod.main()

      assert any("tmux or screen session" in message for message in fake_console.messages), fake_console.messages
    PY

    system libexec/"bin/python3.13", testpath/"test.py"
  end
end
